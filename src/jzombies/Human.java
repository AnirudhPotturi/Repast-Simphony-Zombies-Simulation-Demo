...
public class Human {
	...
	// This is much like the Zombies run method. This won't be scheduled as we did with Zombie class method 'run'.
	// This method looks for cells with zombies and attempts to move towards cells with least
	// The annotation sets up a watcher that watches for whenever a zombie enters a human's neighborhood. The watcher gets triggered whenever the field 'moved' gets triggered.
	// What this means is that whenever a zombie moves and as a result modifies the field "moved", this action action gets triggered for each human.
	...
	{
		// We must only move if we are not in the target location
		...
		if(!point.equals(grid.getLocation(this)))
		{
			...
			// Human moves 2 units instead of 1
